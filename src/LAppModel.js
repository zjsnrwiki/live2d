            ...
            for (var i = 0; i < thisRef.modelSetting.getTextureNum(); i++) {
                ...
                thisRef.loadTexture(i, texPaths, function() {
                    if( thisRef.isTexLoaded ) {
                        ...
                        if (thisRef.eyeBlink == null) thisRef.eyeBlink = new L2DEyeBlink();
                        ...
                        if (thisRef.modelSetting.getPoseFile() != null)
                            thisRef.loadPose(thisRef.modelHomeDir + thisRef.modelSetting.getPoseFile(),
                                function() { thisRef.pose.updateParam(thisRef.live2DModel); });
                        else
                            thisRef.pose = null;
...
LAppModel.prototype.update = function()
{
    ...
    if (!this.mainMotionManager.isFinished())
        l2d_lastIdle = null;
    else if (l2d_lastIdle == null)
        l2d_lastIdle = Date.now();
    else if (Date.now() - l2d_lastIdle > 3000)
        this.startRandomMotion("idle", PRIORITY_IDLE);
    this.live2DModel.loadParam();
    var update = this.mainMotionManager.updateParam(this.live2DModel); 
    if (!update && this.eyeBlink != null) this.eyeBlink.updateParam(this.live2DModel);
    ...
    if (this.lipSync == null) this.live2DModel.setParamFloat("PARAM_MOUTH_OPEN_Y", this.lipSyncValue);
...
{
    var tmp = [];
    for (var name in this.expressions) tmp.push(name);
...
LAppModel.prototype.startMotion = function(name, no, priority)
{
    ...
    if (motionName == null || motionName == "") return;
